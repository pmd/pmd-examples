...
2.  Get the binary distribution of PMD from <https://github.com/pmd/pmd/releases>, e.g. pmd-bin-6.9.0.zip:

        $ wget https://github.com/pmd/pmd/releases/download/pmd_releases%2F6.9.0/pmd-bin-6.9.0.zip

3.  Extract the zip file, e.g. `unzip pmd-bin-6.9.0.zip`

        $ unzip pmd-bin-6.9.0.zip

4.  Now, the pmd binaries are installed under `~/code/pmd-bin-6.9.0`.

    This also includes the libraries in `~/code/pmd-bin-6.9.0/lib`.
...
3.  Compile the sources, that are located in `src`, using the PMD libraries
        ...
        $ javac -d build -cp '../../pmd-bin-6.9.0/lib/*' src/*.java
...
1.  Run PMD with the just created jar file on the classpath, e.g. on the folder `testsrc`
        ...
        $ CLASSPATH=custom-rule-example.jar ../../pmd-bin-6.9.0/bin/run.sh pmd -no-cache -f text -d testsrc -R myrule.xml
